...
// Default dark mode colors for types 0-7
const darkModeColors = [
  '#ffffff', // type 0 - white
  '#5555ff', // type 1 - blue
  '#ff5555', // type 2 - red
  '#aaff55', // type 3 - green-yellow
  '#55ffff', // type 4 - cyan
  '#ffdf55', // type 5 - orange
  '#55ff55', // type 6 - bright green
  '#5599ff'  // type 7 - blue-purple
];
const darkModeDefaultStroke = '#FFFFFF';
const darkModeDefaultFill= '#FFFFFF'; // font color

// Enhanced typeStyle function that supports type_colors and mode configurations
const typeStyle = (t, opt) => {
  // If type_colors is provided in options, use it
  if (opt && opt.type_colors && Array.isArray(opt.type_colors) && opt.type_colors[t] !== undefined) {
    return ';fill:' + opt.type_colors[t];
  }
  
  // If mode is dark, use dark mode colors
  if (opt && opt.mode === 'dark' && darkModeColors[t] !== undefined) {
    return ';fill:' + darkModeColors[t];
  }
  
  // Otherwise use default colors
  return (colors[t] !== undefined) ? ';fill:' + colors[t] : '';
};
    ...
    if ((e.name === undefined) || (e.type !== undefined)) {
      if (!(opt.compact && e.type === undefined)) {
        blanks.push(['rect', Object.assign({}, norm({
          ...
          field: e.name,
          style: 'fill-opacity:'+opt.opacity + typeStyle(e.type, opt)
        }),
...
const getLegendItems = (opt) => {
  ...
  for(const key in legend) {
    ...
    items.push(['rect', norm({
      ...
      style: 'fill-opacity:'+opt.opacity+'; stroke: ' + opt.stroke + '; stroke-width: 1.2;' + typeStyle(value, opt)
    })]);
...
const cage = (desc, opt) => {
  const {hspace, vspace, mod, margin, index, vflip, stroke} = opt;
  const width = hspace - margin.left - margin.right - 1;
  const height = vspace - margin.top - margin.bottom;
  const res = ['g',
    {
...
const optDefaults = opt => {
  ...
  // Add default mode as 'light'
  opt.mode = opt.mode || 'light';
  // opt.stroke = opt.stroke || 'black';
  // opt.fill = opt.fill || 'black';
  // dark模式下, 且没有设置stroke的情况, 就设置默认的dark stroke 颜色
  // 其他情况下, 如果在json中有设置了stroke, 就使用stroke,否则使用 black
  opt.stroke= opt.mode=='dark' && !opt.stroke ? darkModeDefaultStroke :  (opt.stroke || 'black');
  opt.fill = opt.mode=='dark' && !opt.fill ? darkModeDefaultFill : (opt.fill || 'black');
  opt.opacity = opt.mode=='dark' &&!opt.opacity ? 0.5: (opt.opacity||0.15);
...
const render = (desc, opt) => {
  ...
  const {hspace, vspace, lanes, margin, compact, fontsize, bits, label, legend, fill} = opt;
